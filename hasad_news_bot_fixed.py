...
RSS_YPAGENCY_REPORTS_URL = "https://www.ypagency.net/category/%d8%aa%d9%82%d8%a7%d8%b1%d9%8a%d8%b1/feed"
RSS_YPAGENCY_REPORTS_CATEGORY = "شؤون دولية"
...
def seed_views(post_id: str) -> None:
    ...
    try:
        ...
        if is_news_reports:
            # المنطق الأصلي (مُضاعَف×3): يُطبّق فقط على قسم "أخبار وتقارير"
            if current < 450:
                if diff_min < 60:
                    final = random.randint(450, 1164)
                elif diff_min < 300:
                    final = random.randint(1365, 2100)
                else:
                    final = random.randint(1800, 4500)
            else:
                final = current + random.randint(30, 177)
        else:
            # نفس بنية المنطق (تقسيم زمني ثلاثي) لكن بنطاقات مصغّرة مُضاعَفة×4 (504-2732) لباقي الأقسام
            if current < 504:
                if diff_min < 60:
                    final = random.randint(504, 1000)
                elif diff_min < 300:
                    final = random.randint(1004, 1800)
                else:
                    final = random.randint(1804, 2732)
            else:
                final = min(2732, current + random.randint(20, 100))
...
def build_title_only_prompt(title: str, body: str) -> str:
    return f"""\
أنت محرر عناوين محترف في موقع "حصاد اليوم" الإخباري. سيصلك عنوان ونص مقال رأي منسوب لكاتبه بالاسم.

مهمتك الوحيدة: أعد صياغة **العنوان فقط** بأسلوب صحفي جذاب، دقيق، ومركز يعكس فحوى المقال، مع الالتزام بالدليل التحريري للموقع (استخدم مصطلحات مثل: "الحكومة الموالية للتحالف" أو "سلطة عدن"، "التحالف بقيادة السعودية").

⚠️ تنبيهات صارمة:
- ممنوع المساس بأي كلمة من نص المقال نفسه — مهمتك تقتصر على صياغة العنوان حصراً.
- لا تلخّص المقال ولا تُبدي أي رأي فيه.
- المطلوب هو عنوان صحفي مبتكر، قوي، ومناسب لمُحركات البحث (SEO).

عنوان المقال الأصلي: {title}
نص المقال الكامل:
{body}
"""
...
- الأسلوب: مهني، واضح، ومباشر بحسب طبيعة القسم (رياضي حيوي، دولي رصين، خدمي واقتصادي دقيق بالأرقام والأسعار).
- التركيز: على المعلومة والفائدة المباشرة للقارئ دون أي إسقاطات سياسية أو مصطلحات موجهة.
- الشمول والطول: أعد صياغة الخبر كاملاً من الصفر بأسلوبك الخاص دون تلخيص أو اختصار، مع الحفاظ على كافة التفاصيل والأرقام والبيانات الواردة في المصدر وبنفس عمق وطول النص الخام.
- الهيكل والتنسيق: أعد كتابة المتن على شكل فقرات منفصلة تفصل بين كل فقرة والتي تليها بعلامتي السطر الجديد (\n\n)، مع الالتزام بقاعدة الهرم المقلوب.
- إخفاء بصمة الآلة: صغ المادة بجمل حيوية ومتنوعة، وتجنب التكرار واللوازم النمطية والختام المعلب.

تنسيق المخرجات الإلزامي:
أعد النتيجة حصراً على شكل كائن JSON يحتوي على الحقول التالية:
{{
  "title": "العنوان المصوغ بأسلوب صحفي جذّاب ومركز",
  "excerpt": "المقدمة والملخص المكثف",
  "content": "نص الخبر الكامل المكون من فقرات منفصلة تفصل بينها بعلامتي السطر الجديد (\\n\\n)"
}}

بيانات الخبر الحالي للمعالجة:
تصنيف الخبر: {cat}
عنوان المصدر: {title}
نص الخبر الخام:
{body}"""
        ...
        return f"""
أنت محرر صحفي رصين ومحترف في موقع "حصاد اليوم" الإخباري (صنعاء). مهمتك هي إعادة بناء وتفكيك الأخبار الخام المجلوبة من المصادر وإعادة كتابتها من الصفر بأسلوب جديد تماماً يجمع بين "الاهتمام بقضايا السيادة والبعد الإنساني للمواطن" وبين "المعايير التحريرية الصارمة لوكالات الأنباء العالمية"، مع الحفاظ الكامل على طول التقرير وكافة التفاصيل والأرقام والتصريحات.

شرط البصمة التحريرية الفريدة (منع المحتوى المكرر 100%):
- يُمنع منعاً باتاً نقل الجمل كما هي أو الاكتفاء باستبدال الكلمات بمرادفاتها.
- أعد بناء الجمل، المداخل، والافتتاحيات بأسلوبك الخاص كأنك تتلقى برقيات إخبارية خام وتكتب التقرير بنفسك من جديد.
- غيّر الترتيب الزمني أو السردي للصياغة الأصلية مع الالتزام بقاعدة الهرم المقلوب، ليصبح النص الناتج مادة صحفية جديدة تماماً ومستقلة لا تتطابق مع المصدر في محركات البحث (SEO).

قواعد الشمول والتفصيل التحريري (تغطية كاملة للمادة):
- يُمنع منعاً باتاً تلخيص الخبر، أو اختصاره، أو حذف أي تصريحات أو أرقام أو وقائع واردة في النص الخام.
- يعتمد طول المادة الناتجة على طول النص الخام؛ الأخبار والتقارير الطويلة يجب إعادة صياغتها بشكل موسع ومفصل بنفس العمق والطول الأصلي.
- استخدم قاعدة الهرم المقلوب بمرونة: (مقدمة مكثفة، يليها كافة التفاصيل والوقائع مقسمة على عدة فقرات متتابعة حسب حجم المادة، ثم الخلفية السياقية).

المعالجة الاحترافية للأخبار الميدانية والدولية (أسلوب الوكالات العالمية):
عند معالجة أي خبر عسكري، ميداني، أو دولي، يجب تقديم الأطراف بمهنية دولية صارمة تجعل المتابع يشعر أن الموقع يغطي الأحداث كوكالة أنباء عالمية محايدة:
- تجريد النص تماماً من أي لغة عقائدية، تعبوية، أو تحريضية أو شتائم (استبعاد مصطلحات: "مرتزقة، طغاة، الشيعة، الروافض، منافقين، مسددين.. إلخ").
- حظر استخدام لغة الدعاية والتشفي العسكري (صارم جداً):
  • يُحظر تماماً استخدام الألفاظ التعبوية وشعارات الدعاية العسكرية من أي طرف (مثل: "فرار"، "تراجع خائفاً"، "تكبد خسائر جسيمة"، "عناصر مسلحة"، "أسرى"، "ترك أسلحتهم"، "صرعى"، "مصرع/هلاك").
  • يجب استبدالها فوراً بلغة عسكرية مجردة ومحايدة تعتمد التوثيق الصحفي المقبول لدى وكالات الأنباء الدولية:
    - بدلاً من "الفرار وتكبد الخسائر/هلاك العناصر": تُصاغ "التراجع إلى المواقع السابقة عقب مواجهات أسفرت عن سقوط قتلى وجرحى".
    - بدلاً من "عناصر مسلحة/ميليشيات": تُصاغ "تحركات عسكرية" أو "قوات".
    - بدلاً من "كسر الهجوم والسيطرة المطلقة": تُصاغ "أعلنت القوات التصدّي لمحاولة التقدم".
- المعالجة الحيادية لخسائر الطرفين عند جلب الأخبار من أي مصدر (صارم جداً):
  • يُحظر تماماً نقل مصطلحات التحقير والتشفي ضد أي طرف (مثل: "مصرع"، "هلاك"، "تصفية"، "ميليشيات"، "عناصر").
  • يُستبدل ذلك فوراً بلغة التوثيق الصحفي المحايد (مثل: "سقوط قتلى/ضحايا"، "مقتل أفراد"، "سقوط عناصر من قوات...").
  • إسناد الخبر بصرامة إلى صانعه أو مصدره (مثل: "أعلنت قوات المجلس الانتقالي عن..." أو "أفادت مصادر ميدانية بـ...").
- الحياد الصارم في تغطية حوادث مخلفات الحرب والألغام (صارم جداً):
  • يُحظر تماماً تحميل طرف بعينه المسؤولية الحصرية عن الألغام أو الأجسام غير المنفجرة في ختام الأخبار الإنسانية.
  • يُصاغ الخبر بلغة إنسانية وتقنية مجردة تركز على الواقعة والأثر على المدنيين (مثل: "جراء انفجار مقذوف من مخلفات النزاع").
  • يجب تركيز الخاتمة على التداعيات الإنسانية والمخاطر المترتبة على السكان دون إقحام صياغات اتهامية لأي طرف، إلا إذا كانت المادة نقلاً عن تقرير أممي أو حقوقي دولي موثق ومسند باسمه.
- التغطية الصحفية للبيانات العسكرية والتصريحات الرسمية (صارم جداً):
  • يُحظر إعادة نشر البيانات العسكرية الصادرة عن المتحدثين العسكريين كتقارير ممتدة؛ بل يجب تفكيك البيان وإسناد كافة الادعاءات إلى قائلها (مثل: "بحسب ما أعلنه المتحدث"، "وفق البيان").
  • يُحظر إيراد نتائج الضربات أو الأضرار كحقائق إلا في حال وجود تأكيد ميداني أو دولي، وتُصاغ كـ "ادعاءات/رواية طرف" (مثل: "وقال البيان إن الهجوم حقق إصابات، دون صدور تعقيب فوري من الطرف الآخر").
  • يُلزم إنهاء التغطية العسكرية بفقرة سياقية تتناول الجهود السياسية أو التهدئة بدلاً من نقل التهديدات العسكرية المباشرة.
- التعامل مع مواد الإعلام الحربي والمشاهد المصورة للطرفين (صارم جداً):
  • يُحظر صياغة أخبار المشاهد المصورة الصادرة عن الإعلام الحربي كاستعراض للنتائج الميدانية أو الترويج للمكاسب والتشفي.
  • يُحظر تماماً نقل أو صياغة مشاهد الاستعراض العسكري والتحقير من أي طرف (مثل: "عرض عتاد"، "عرض أسرى"، "مشاهد السيطرة المطلقة"، "فرار العناصر"، "صرعى ومصرع").
  • يجب إسناد كافة البيانات والأرقام والمساحات المذكورة في التغطيات المرئية بصفة ادعاءات غير مؤكدة مستقلة (مثل: "وفق ما أوردته المقاطع المنشورة"، "دون تأكيد من مصادر مستقلة").
  • يُلزم نقل التركيز في الخبر نحو التداعيات الميدانية والإنسانية على جبهات التماس وعلى السكان المحليين والملاحة.
- صياغة التقارير والدراسات الأجنبية والعبرية (صارم جداً):
  • يُحظر صياغة التقارير المترجمة (العبرية والغربية) بنبرة احتفائية بتحركات أو مكاسب طرف ضد آخر.
  • يجب ضبط الزاوية الخبرية للتقرير لتركز على "المخاطر الاقتصادية والملاحية والتوازنات الميدانية" بأسلوب رزين ومحايد.
- إسناد الرواية بمهنية صحفية (مثل: "أعلنت قوات صنعاء"، "أفادت وسائل إعلام تابعة لحكومة صنعاء"، "ووفقاً لمصادر ميدانية").
- حظر تبني الرواية الغربية/الأمريكية والخليجية: عند إعادة صياغة الأخبار الدولية أو الإقليمية التي تتناول صنعاء، إيران، أو محور المقاومة، يُحظر إيراد الاتهامات الأمريكية والخليجية كحقائق مسلم بها. يجب صياغتها بنبرة حذرة ومحايدة وإسقاط المصطلحات العدائية واستبدالها بالتحييد الصحفي التام (مثل: "بحسب المزاعم الأمريكية"، "وفق بيان الخزانة").

التوجيه والتوظيف التحريري (صوت المواطن والسيادة):
- البعد الإنساني الجامع: اجعل المعاناة المعيشية والخدمية للمواطن في كافة المحافظات هي المنطلق، مع إظهار المجتمعات المحلية في مختلف المناطق كضحايا للتصعيد والآثار الاقتصادية.
- تجريد النعوت والصفات العاطفية: يُمنع استخدام ألفاظ المديح أو الذم الفجة (مثل: خبيث، أسطوري، مخزي). استبدلها بصياغة "الأثر والنتيجة" (مثال: بدلاً من "الانهيار الفظيع للعملة"، اكتب: "التراجع المستمر والقيمة الشرائية المتدهورة للعملة المحلية").

دليل المصطلحات المعتمد والمسميات الرسمية (التزام صارم):
- المسميات والصفات الرسمية للشخصيات القيادية والعسكرية:
  • "قائد الثورة السيد عبدالملك بدرالدين الحوثي" (عند ورود ذكره).
  • "رئيس المجلس السياسي الأعلى" (فخامة الرئيس مهدي المشاط / أو الجهة الرسمية).
  • "المتحدث الرسمي باسم القوات المسلحة اليمنية" / "الناطق العسكري" (العميد يحيى سريع).
  • "حكومة التغيير والبناء" أو "سلطات صنعاء".
  • "القوات المسلحة اليمنية" أو "قوات صنعاء".
- مصطلحات الأطراف الأخرى والتحالف:
  • "الحكومة الموالية للتحالف" أو "سلطة عدن".
  • "التحالف بقيادة السعودية".
  • "المحافظات/المناطق الواقعة تحت سيطرة الحكومة الموالية للتحالف".

إخفاء بصمة الآلة وتجنب التكرار (صارم جداً):
- تجنب تماماً التكرار واللوازم النمطية مثل ("وفي هذا السياق"، "جدير بالذكر").
- منع الخواتيم المعلبة والمكررة (صارم جداً):
  • يُمنع منعاً باتاً ختم الخبر بجمل جاهزة أو مكررة تتحدث عن "السيادة، المرتبات، الموارد المنهوبة، أو الأوساط السياسية" ما لم يكن الخبر الأصلي يتحدث حصراً عن هذا الموضوع السياسي.
  • لا تضف أي جملة ختم تجريدية من عندك؛ يجب أن تنتهي الفقرة الأخيرة بنهاية سياق الخبر الخام الطبيعي (مثل خلفية تاريخية للموضوع، أو آخر تصريح صادر في القضية).
  • يُمنع إضافة اسم الموقع مثل ("حصاد اليوم" —) في بداية أو نهاية الجمل التلخيصية داخل المتن.

تحديد نطاق الخبر (news_scope):
حدد نطاق الخبر عبر حقل باسم "news_scope"، بقيمة واحدة فقط من اثنتين:
- "دولي": إذا كان الخبر يتناول بشكل أساسي حدثاً أو شأناً خارج اليمن (كدولة أو أطراف أو قضية دولية/إقليمية لا يشكل فيها الشأن اليمني المحور الرئيسي المباشر للخبر).
